...
func Query(conn net.Conn, query string, runes []rune, single bool, exact bool, _ uint8) []*pb.QueryResponse_Item {
	...
	for k, v := range actions {
		...
		if query != "" {
			match, score, positions, start, found := calcScore(runes, slab, k, v, exact)
...
func calcScore(runes []rune, slab *common.FuzzySlab, k, v string, exact bool) (string, int32, *[]int, int32, bool) {
	var scoreRes int32
	var posRes *[]int
	var startRes int32
	var match string
